...
fn create_corpus() {
    ...
    let mut playground = NetworkPlayground::new(static_runtime.executor());
    let nodes = NodeSetup::create_nodes(&mut playground, static_runtime.executor(), 2, true);
    let node = &nodes[0];
...
fn fuzz_single_file() {
    ...
    let mut buf = [0; 1000];
    let mut f =
        File::open("/Users/davidwg/libra_internal/libra/consensus/fuzz/corpus/proposal.bin")
            .unwrap();
...
fn fuzz_artifacts() {
    let paths = fs::read_dir(
        "/Users/davidwg/libra_internal/libra/consensus/fuzz/artifacts/fuzz_consensus_proposal/",
    )
    .unwrap();
...
fn fuzz(data: &[u8]) {
    ...
    let mut playground = NetworkPlayground::new(static_runtime.executor());
    let mut node = NodeSetup::create_nodes(&mut playground, static_runtime.executor(), 1, false)
        .pop()
        .unwrap();
